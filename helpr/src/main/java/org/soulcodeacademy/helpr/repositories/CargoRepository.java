...
public interface CargoRepository extends JpaRepository<Cargo, Integer> { // <Entidade, PK>
    List<Cargo> findByNome(String nome); //SELECT * FROM cargo WHERE nome = valor;
    List<Cargo> findBySalario(Double valor); // SELECT * FROM cargo WHERE salario = valor
